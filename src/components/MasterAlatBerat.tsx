...
import { Plus, Search, Trash2, Edit3, Settings, Upload, FileSpreadsheet } from 'lucide-react';
import Modal from './Modal';
import * as XLSX from 'xlsx';

interface MasterAlatBeratProps {
  data: AlatBerat[];
  onAdd: (item: Omit<AlatBerat, 'id'>) => void;
  onUpdate: (item: AlatBerat) => void;
  onDelete: (id: string) => void;
  onImportClick?: () => void;
}

export default function MasterAlatBerat({
  data,
  onAdd,
  onUpdate,
  onDelete,
  onImportClick,
}: MasterAlatBeratProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<AlatBerat | null>(null);

  // Form State
  const [namaAlat, setNamaAlat] = useState('');
  const [kategori, setKategori] = useState<AlatBerat['kategori']>('Excavator');
  const [sewaJam, setSewaJam] = useState<number>(0);

  // Helper to generate next code
  const getNextCode = () => {
    const codes = data.map((x) => x.kodeAlat);
    const prefix = 'AB/';
    const numbers = codes
      .map((code) => {
  ...
  const handleOpenAddModal = () => {
    setEditingItem(null);
    setNamaAlat('');
    setKategori('Excavator');
    setSewaJam(0);
    setIsModalOpen(true);
  };

  const handleOpenEditModal = (item: AlatBerat) => {
    setEditingItem(item);
    setNamaAlat(item.namaAlat);
    setKategori(item.kategori);
    setSewaJam(item.sewaJam);
    setIsModalOpen(true);
  };
  ...
  const handleExportExcel = () => {
    const formattedRows = filteredData.map((item) => ({
      'Kode Alat': item.kodeAlat,
      'Nama Alat': item.namaAlat,
      'Kategori': item.kategori,
      'Harga Sewa / Jam (Rp)': item.sewaJam,
    }));

    const worksheet = XLSX.utils.json_to_sheet(formattedRows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Master Alat Berat');
    ...
    XLSX.writeFile(workbook, `Master_Alat_Berat_Maju_Jaya_${todayStr}.xlsx`);
  };
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:items-center justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            id="search-alat-berat"
            type="text"
            placeholder="Cari Alat Berat (Kode, Nama, Kategori)..."
            value={searchTerm}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-100 border-b border-slate-300 text-slate-600 text-[10px] font-bold uppercase tracking-wider">
                <th className="px-4 py-2 border-r border-slate-200">Kode Alat</th>
                <th className="px-4 py-2 border-r border-slate-200">Nama Alat</th>
                <th className="px-4 py-2 border-r border-slate-200">Kategori</th>
                <th className="px-4 py-2 text-right border-r border-slate-200">Sewa / Jam</th>
              ...
              {filteredData.length > 0 ? (
                filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-1.5 font-semibold text-slate-800 border-r border-slate-200">{item.namaAlat}</td>
                    <td className="px-4 py-1.5 border-r border-slate-200">
                      <span className="text-[9px] bg-slate-200 text-slate-700 px-1 py-0.2 rounded-xs font-semibold uppercase">
                        {item.kategori}
                      </span>
                    </td>
                    <td className="px-4 py-1.5 text-right font-mono font-bold text-slate-800 border-r border-slate-200">
                      Rp {item.sewaJam.toLocaleString('id-ID')}
                    </td>
                    <td className="px-4 py-1.5 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          id={`btn-edit-ab-${item.id}`}
                        ...
                        <button
                          id={`btn-delete-ab-${item.id}`}
                          onClick={() => {
                            if (confirm(`Yakin ingin menghapus ${item.namaAlat}?`)) {
                              onDelete(item.id);
                            }
                          }}
                ...
                <tr>
                  <td colSpan={5} className="text-center py-8 text-slate-400">
                    Tidak ada data alat berat ditemukan.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Modal
        ...
        title={editingItem ? 'Ubah Data Alat Berat' : 'Tambah Data Alat Berat'}
      >
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div>
            <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider mb-1">
              Kode Alat (Running Number)
            </label>
            <input
              type="text"
              readOnly
              value={editingItem ? editingItem.kodeAlat : getNextCode()}
            ...
            <p className="mt-0.5 text-[10px] text-slate-400">
              * Diisi otomatis oleh sistem dengan pola AB/[Running Number]
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select
                value={kategori}
                onChange={(e) => setKategori(e.target.value as AlatBerat['kategori'])}
            ...
            <div>
              <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider mb-1">
                Sewa / Jam (Rp) <span className="text-rose-500">*</span>
              </label>
              <input
                type="number"
                required
                ...
                value={sewaJam || ''}
                onChange={(e) => setSewaJam(Math.max(0, parseInt(e.target.value) || 0))}
